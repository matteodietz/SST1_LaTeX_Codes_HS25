...
\underline{\textbf{123}}, \underline{\textbf{124}}, \underline{\textbf{125}}, \underline{\textbf{126}}, \underline{\textbf{127}}, \underline{\textbf{128}}, \underline{\textbf{130}}, \underline{\textbf{131}}\\
\vspace{-0.5cm}
...
$$\hat{x}[k] := \hat{x}\left(\frac{k}{N}\right) = \sum_{n=0}^{N-1} x[n]e^{-2 \pi i \frac{k}{N}n} $$
...
Die Spalten von $F_N$ sind paarweise orthogonal. Sei $\mathbf{f}_i$ die $i-$te Spalte von $F_N$. 

Dann $\langle \mathbf{f}_r, \; \mathbf{f}_s \rangle = N\delta_{r,s}$. Es gilt $F_N F_N^H = N I_N$, wobei $I_N$ die Identitätsmatrix der Dimension $N$ ist.
...
\begin{enumerate}
    ...
    \item Dies entspricht im zeitkontinuierlichen Fall der Eigenschaft, dass die
      Funktionen $e^{2 \pi i f_0 t}$ Eigenfunktionen von LTI-Systemen sind und $\hat{h}(f)$ die dazugehörigen Eigenwerte. Anstelle von $e^{2 \pi i f_0 t}$ haben wir hier die Spalten von $\frac{1}{\sqrt{N}}\mathbf{F}_N^H$ und anstelle von $\hat{h}(f)$ die Werte $\hat{h}[k] = \hat{x}_2[k]$.
